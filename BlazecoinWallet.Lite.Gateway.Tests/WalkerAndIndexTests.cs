...
public class WalkerAndIndexTests : IDisposable
{
    ...
    [Fact]
    public async Task A_post_quantum_output_the_daemon_cannot_decode_is_indexed_under_its_BQ_address()
    {
        // A P2PQH template (0x20 <pqkh:32> 0xba) — a pre-fork daemon serves it with no
        // address at all. The gateway must recognise the template from the hex, index it
        // under the BQ address, and hand the script back to wallets.
        var pqkh = Enumerable.Range(1, 32).Select(i => (byte)i).ToArray();
        var script = Lite.Pq.PqScript.BuildP2pqh(pqkh);
        var hex = Convert.ToHexString(script).ToLowerInvariant();
        var bq = Lite.Pq.PqAddress.Encode(pqkh);
        Assert.StartsWith("BQ", bq);

        _rpc.AddBlock(0, "g");
        _rpc.AddBlock(1, "b1", FakeDaemonRpc.Spend(FakeDaemonRpc.TxId("pay-pq"),
            (FakeDaemonRpc.TxId("cb0"), 0), FakeDaemonRpc.OutHexOnly(0, hex, 7_000_000), FakeDaemonRpc.Out(1, Alice, 1_000)));
        // An OP_RETURN-style output with no address and a non-template hex stays out of the index.
        _rpc.AddBlock(2, "b2", FakeDaemonRpc.Spend(FakeDaemonRpc.TxId("burn"),
            (FakeDaemonRpc.TxId("cb1"), 0), FakeDaemonRpc.OutHexOnly(0, "6a04deadbeef", 1)));
        await Tick();

        var u = Assert.Single(_db.GetUtxos(bq));
        Assert.Equal(7_000_000, u.Amount);
        Assert.Equal(hex, u.ScriptPubKey);
        Assert.Equal(1, _db.GetSummary(bq)!.TxCount);
        Assert.Null(Assert.Single(_db.GetUtxos(Alice)).ScriptPubKey); // legacy row from an address-only daemon shape

        // The same recognition in the mempool overlay (0-conf incoming to a BQ address).
        var memTx = FakeDaemonRpc.Spend(FakeDaemonRpc.TxId("mem-pq"),
            (FakeDaemonRpc.TxId("cb2"), 0), FakeDaemonRpc.OutHexOnly(0, hex, 2_000_000));
        _rpc.MempoolTxs[memTx.TxId] = memTx;
        await Tick();
        var m = Assert.Single(_mempool.GetForAddress(bq));
        Assert.Equal(2_000_000, m.Amount);
        Assert.Equal(hex, m.ScriptPubKey);

        // Regtest prefix when the gateway is configured for regtest.
        var tqDb = new UtxoIndexDb(_dbPath + ".reg", mainnet: false);
        try
        {
            tqDb.ApplyBlock(_rpc.Blocks[0]);
            tqDb.ApplyBlock(_rpc.Blocks[1]);
            var tq = Lite.Pq.PqAddress.Encode(pqkh, BlazecoinNetwork.Regtest);
            Assert.StartsWith("TQ", tq);
            Assert.Single(tqDb.GetUtxos(tq));
            Assert.Empty(tqDb.GetUtxos(bq));
        }
        finally
        {
            Microsoft.Data.Sqlite.SqliteConnection.ClearAllPools();
            File.Delete(_dbPath + ".reg");
        }
    }
