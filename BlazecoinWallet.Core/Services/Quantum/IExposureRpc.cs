...
/// <summary>Progress for the scan: which phase, which transaction of how many, and keys found so far.</summary>
public readonly record struct ExposureProgress(int TransactionsDone, int TransactionsTotal, int ExposedKeysFound,
    ExposurePhase Phase = ExposurePhase.CheckingSends);

/// <summary>The scan's two phases: one long read of the wallet's history, then one read per send.</summary>
public enum ExposurePhase { ReadingHistory, CheckingSends }
