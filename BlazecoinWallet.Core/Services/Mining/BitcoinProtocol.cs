...
public static class BitcoinProtocol
{
    ...
    /// <summary>Blazecoin mainnet P2PKH version byte — the 'B' in every legacy address (chainparams PUBKEY_ADDRESS = 26).</summary>
    public const byte MainnetPubKeyVersion = 26;

    /// <summary>Two-byte version prefix of a post-quantum P2PQH address (PQ_SIGNATURES.md §5):
    /// Base58Check(0x46 0x50 ‖ pqkh[32]) — 52 characters, always "BQ…".</summary>
    public static readonly byte[] MainnetPqAddressPrefix = { 0x46, 0x50 };

    /// <summary>True for a well-formed mainnet post-quantum "BQ…" address (prefix, 32-byte
    /// key hash, valid checksum). Such an address is a TAGGED HASH of an ML-DSA-44 key: a
    /// spend reveals the key, but Shor's algorithm gains nothing from a lattice key, so the
    /// Quantum Exposure page classifies it as post-quantum rather than exposed/unexposed.</summary>
    public static bool IsPostQuantumAddress(string? address)
    {
        if (string.IsNullOrWhiteSpace(address) || address.Length != 52) return false;
        byte[] payload;
        try { payload = Base58CheckDecode(address.Trim()); }
        catch (FormatException) { return false; }
        return payload.Length == 34 && payload[0] == MainnetPqAddressPrefix[0] && payload[1] == MainnetPqAddressPrefix[1];
    }

    /// <summary>RIPEMD160(SHA256(data)) — the 20-byte pubkey hash inside every P2PKH address.</summary>
    public static byte[] Hash160(byte[] data)
    {
        ...
        var output = new byte[ripe.GetDigestSize()];
        ripe.DoFinal(output, 0);
        return output;
    }

    /// <summary>The legacy address a public key pays to: base58check(version ‖ HASH160(pubkey)).</summary>
    public static string PubKeyToAddress(byte[] pubKey, byte version = MainnetPubKeyVersion)
    {
        var payload = new byte[21];
        payload[0] = version;
        Buffer.BlockCopy(Hash160(pubKey), 0, payload, 1, 20);
        return Base58CheckEncode(payload);
    }

    /// <summary>Base58check: payload ‖ first 4 bytes of SHA256d(payload), base58-encoded with leading-zero '1's.</summary>
    public static string Base58CheckEncode(byte[] payload)
    {
        var checksum = DoubleSha256(payload);
        var raw = new byte[payload.Length + 4];
        Buffer.BlockCopy(payload, 0, raw, 0, payload.Length);
        Buffer.BlockCopy(checksum, 0, raw, payload.Length, 4);
    ...
    private static string Base58Encode(byte[] data)
    {
        ...
        while (bigInt > 0)
        {
            bigInt = System.Numerics.BigInteger.DivRem(bigInt, 58, out var rem);
