...
public class BlazecoindRpcService : IBlazecoindRpcService, Provenance.IAncestryRpc, Provenance.IVintageSendRpc, Quantum.IExposureRpc
{
    ...
    public async Task<List<PeerSummary>> GetPeersAsync()
    {
        ...
        return peers?
            ...
            .Select(p => new PeerSummary(p.Addr!, string.IsNullOrWhiteSpace(p.Subver) ? "unknown" : p.Subver!,
                                         p.SyncedHeaders, p.PresyncedHeaders, p.StartingHeight))
    ...
    public async Task CreateWalletAsync(string name)
    {
        // Node endpoint on purpose: on a fresh install nothing is active to scope to, and the
        // call creates rather than targets a wallet. Core 28 makes a descriptor wallet and loads it.
        await CallRpcAtAsync<LoadWalletResult>(_rpcUrl, "createwallet", LongWalletOpTimeout, name);
    }
    ...
    private class PeerInfo
    {
        ...
        // Sync signals. During Core's headers PRESYNC (its first pass over a fresh chain) getblockchaininfo
        // still reports headers=0 — only these per-peer fields move (2026-09-24, Intel Mac first run).
        public int StartingHeight { get; set; } = -1;
        [System.Text.Json.Serialization.JsonPropertyName("synced_headers")] public int SyncedHeaders { get; set; } = -1;
        [System.Text.Json.Serialization.JsonPropertyName("presynced_headers")] public int PresyncedHeaders { get; set; } = -1;
    }
}
...
/// <summary>One getpeerinfo row. The three heights are -1 when unknown: <c>StartingHeight</c> is the
/// peer's tip when it connected (a cheap network-tip estimate), <c>SyncedHeaders</c> what it has served
/// us, <c>PresyncedHeaders</c> how far Core's headers presync has got with it.</summary>
public record PeerSummary(string Address, string SubVersion, int SyncedHeaders = -1, int PresyncedHeaders = -1, int StartingHeight = -1);
