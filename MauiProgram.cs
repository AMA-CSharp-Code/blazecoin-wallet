...
public static class MauiProgram
{
	...
	public static MauiApp CreateMauiApp()
	{
		...
		// QuestPDF is deliberately NOT touched here. Setting its licence at startup ran its
		// native-dependency check, and on a Mac older than macOS 15 (the floor of the shipped
		// libQuestPdfSkia) that killed the app before any UI appeared (Intel Mac, 2026-09-24).
		// TransactionExportService sets the licence on first PDF export and fails that export
		// alone if the native library can't load.
...
#if MACCATALYST
		// macOS (2026-09-22): closing the window leaves the app — and its node — running
		// in the Dock, so the Windows close hook above has no Catalyst twin. Quit (Cmd-Q)
		// must not orphan a node WE launched, though. WillTerminate gives a few seconds:
		// enough to send the graceful RPC stop (SIGTERM fallback mid-warmup); the daemon
		// is its own process and finishes its flush after we are gone. An adopted node is
		// left alone, as on Windows. Task.Run keeps the awaits off the (blocked) UI
		// thread's SynchronizationContext, or the bounded Wait would deadlock.
		builder.ConfigureLifecycleEvents(events =>
		{
			events.AddiOS(ios => ios.WillTerminate(_ =>
			{
				var mgr = _appServices?
					.GetService(typeof(IDaemonProcessManager)) as IDaemonProcessManager;
				if (mgr is null || !mgr.WeStartedIt) return;
				try { Task.Run(() => mgr.StopIfOwnedAsync()).Wait(TimeSpan.FromSeconds(5)); }
				catch { /* best-effort — the stop was sent; the node completes it alone */ }
			}));
		});
#endif
		...
		// The wallet-directory resolver honours a --datadir override (same rule as
		// RpcCredentials): the direct-migration path — used on daemons built without
		// Berkeley DB and for BDB-18 (version 10) files — places the legacy file where
		// the daemon will look for it.
		builder.Services.AddSingleton<IWalletImportService>(sp =>
			new WalletImportService(
				sp.GetRequiredService<IWalletRpc>(),
				sp.GetRequiredService<IWalletContext>(),
				() =>
				{
					var dir = sp.GetRequiredService<IConfiguration>()["Blazecoind:DataDir"];
					var dataDir = string.IsNullOrWhiteSpace(dir)
						? BlazecoinWallet.Core.Services.DaemonPaths.DefaultDataDir()
						: dir;
					return Path.Combine(dataDir, "wallets");
				}));
