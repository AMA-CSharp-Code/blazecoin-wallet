...
public class QuantumExposureTests
{
    ...
    public async Task Scanner_flags_the_address_whose_key_appeared_in_a_send_and_leaves_the_rest_unexposed()
    {
        ...
        // The first report announces the history read, so a large wallet never looks frozen at zero.
        Assert.Equal(ExposurePhase.ReadingHistory, progress[0].Phase);
        Assert.Equal(ExposurePhase.CheckingSends, progress[^1].Phase);
    }

    [Fact]
    public async Task Scanner_classifies_a_BQ_address_as_post_quantum_never_exposed()
    {
        // PQ_SIGNATURES.md §5 example: pqkh = 0x00…00 → BQGeaQms…; and a BQ address the
        // encoder builds from the 0x46 0x50 prefix over a 32-byte key hash.
        const string specExample = "BQGeaQmsowAjL1ZG8q1AfVH4NgBvtJKJSsjnnKEf62BMjekdU82n";
        var payload = new byte[34]; payload[0] = 0x46; payload[1] = 0x50;
        for (int i = 0; i < 32; i++) payload[2 + i] = (byte)(i + 1);
        var bq = BitcoinProtocol.Base58CheckEncode(payload);
        Assert.StartsWith("BQ", bq);
        Assert.Equal(52, bq.Length);
        Assert.True(BitcoinProtocol.IsPostQuantumAddress(specExample));
        Assert.True(BitcoinProtocol.IsPostQuantumAddress(bq));
        Assert.False(BitcoinProtocol.IsPostQuantumAddress(RealAddress));                 // legacy B…
        Assert.False(BitcoinProtocol.IsPostQuantumAddress(specExample[..^1] + "m"));    // checksum
        Assert.False(BitcoinProtocol.IsPostQuantumAddress(null));

        var rpc = new FakeRpc();
        rpc.Unspent.Add(new UnspentOutput("t1", 0, 5_000_000_000, RealAddress, 50));
        rpc.Unspent.Add(new UnspentOutput("t2", 0, 3_000_000_000, bq, 60));
        rpc.Unspent.Add(new UnspentOutput("t3", 0, 1_000_000_000, SeqHashAddress, 90));
        rpc.ScriptSigsByTx["s1"] = new List<string> { RealScriptSigHex };

        var report = await new QuantumExposureScanner(rpc).ScanAsync();

        var pq = report.Addresses.Single(a => a.Address == bq);
        Assert.Equal(ExposureStatus.PostQuantum, pq.Status);
        Assert.Null(pq.ExposingTxId);
        Assert.Equal(9_000_000_000, report.TotalSatoshis);
        Assert.Equal(5_000_000_000, report.ExposedSatoshis);
        Assert.Equal(3_000_000_000, report.PostQuantumSatoshis);
        Assert.Equal(1_000_000_000, report.UnexposedSatoshis);
        Assert.Equal(1, report.ExposedAddressCount);
        Assert.Equal(1, report.UnexposedAddressCount);
        Assert.Equal(1, report.PostQuantumAddressCount);
        Assert.Equal(ExposureStatus.Exposed, report.Addresses[0].Status);   // exposed rows still sort first
    }
