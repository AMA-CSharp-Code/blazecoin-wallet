...
public abstract class LiteUiTestBase : TestContext
{
        ...
        IQrScanner? scanner = null, IUnitSettings? units = null)
    {
        ...
        // §7.1 fork banner: a no-gateway poller stays silent (State None) so the layout renders
        // no banner; a test registers its own stub AFTER Wire (last registration wins).
        Services.AddSingleton<BlazecoinWallet.Lite.Fork.IForkStatusService>(
            new BlazecoinWallet.Lite.Fork.GatewayForkStatusService(null));
        return svc;
    }
}
