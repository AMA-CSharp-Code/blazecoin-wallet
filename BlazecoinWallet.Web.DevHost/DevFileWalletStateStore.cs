...
public sealed class DevFileWalletStateStore : IWalletStateStore
{
    ...
    private static readonly string PqPath = System.IO.Path.Combine(Dir, "dev-pq-count.txt");
    ...
    public Task<int> GetPqAddressCountAsync() => ReadCountAsync(PqPath);
    public Task SetPqAddressCountAsync(int count) => WriteCountAsync(PqPath, count);
    ...
    public Task ClearAsync()
    {
        ...
        if (File.Exists(PqPath)) File.Delete(PqPath);
