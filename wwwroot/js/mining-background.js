...
function paintQuantumTiles(host) {
    ...
    function drawAtom(a, t) {
        ...
        for (let k = 0; k < 3; k++) {
            ...
            ctx.strokeStyle = 'rgba(255, 90, 90, 0.22)'; ctx.lineWidth = Math.max(1, a.r / 48); ctx.stroke();
            const ang = a.phase + t * a.speed * 1.6 + k * 2.1;
            ctx.beginPath(); ctx.arc(Math.cos(ang) * a.r, Math.sin(ang) * a.r * 0.38, Math.max(2.2, a.r * 0.045), 0, Math.PI * 2);
        ...
        const ns = Math.max(1, a.r / 48);
        const n = [[0, 0], [3.2 * ns, -1.8 * ns], [-2.6 * ns, 2.4 * ns], [1.4 * ns, 3.4 * ns]];
        for (let j = 0; j < n.length; j++) {
            ctx.beginPath(); ctx.arc(n[j][0], n[j][1], 3 * ns, 0, Math.PI * 2);
