...
// ---------- Navigation probe (diagnostic) ----------
// For each sidebar click: ms until the router swaps the page content in (Blazor/.NET side),
// then 3 s of frame timing after that (WebKit paint side): worst frame gap, frames over 100 ms,
// and when frames settled under 50 ms. Last 30 entries in localStorage 'nav_probe', readable
// from outside the app via WebKit's localstorage.sqlite3 (Intel Mac diagnosis, 2026-09-24).
(function () {
    if (!window.MutationObserver || !window.requestAnimationFrame) return;
    let pending = null;
    function save(entry) {
        try {
            const list = JSON.parse(localStorage.getItem('nav_probe') || '[]');
            list.push(entry); while (list.length > 30) list.shift();
            localStorage.setItem('nav_probe', JSON.stringify(list));
        } catch (e) { /* storage unavailable */ }
    }
    function watchFrames(entry, t0) {
        let last = performance.now(), worst = 0, over100 = 0, settled = null, calm = 0, frames = 0;
        function tick(now) {
            const gap = now - last; last = now; frames++;
            if (gap > worst) worst = gap;
            if (gap > 100) over100++;
            if (gap < 50) { if (++calm >= 5 && settled === null) settled = Math.round(now - t0); } else calm = 0;
            if (now - t0 < 3000) requestAnimationFrame(tick);
            else { entry.worstFrameMs = Math.round(worst); entry.framesOver100 = over100; entry.settledMs = settled; entry.frames = frames; save(entry); }
        }
    ...
    const mo = new MutationObserver(function () {
        ...
        p.entry.swapMs = Math.round(now - p.t0);
        watchFrames(p.entry, p.t0);
    });
    ...
    document.addEventListener('click', function (ev) {
        const a = ev.target && ev.target.closest ? ev.target.closest('a.nav-link, a.navbar-brand') : null;
        if (!a) return;
        pending = { t0: performance.now(), entry: { to: a.getAttribute('href') || '', from: location.pathname, at: new Date().toISOString() } };
        setTimeout(function () { if (pending) { pending.entry.swapMs = null; save(pending.entry); pending = null; } }, 10000);
    }, true);
})();

// Image URLs go through window.__assetBase when the host sets one. On Mac Catalyst MainPage sets
// 'blz://assets/', a scheme handler that serves wwwroot with cacheable headers, because MAUI's
// app:// handler stamps Cache-Control: no-store on everything and WebKit then re-fetches and
// re-decodes every tile on every page visit (Intel Mac: images visibly slow, 2026-09-24).
// Elsewhere __assetBase is unset and the URL is unchanged.
function assetUrl(path) { return (window.__assetBase || '') + path; }

function splashTrackTotal(n) { window.__preload.total += n; }
function splashTrackOne() { window.__preload.done++; maybeDismissSplash(); }
function splashTrackError() { window.__preload.errors = (window.__preload.errors || 0) + 1; splashTrackOne(); }
// Diagnostic breadcrumb, readable from outside the app via WebKit's localstorage.sqlite3: which
// asset base the tile painters used and how the startup preloads fared (added 2026-09-24 to
// verify the blz:// cacheable scheme on a Mac without the Web Inspector).
function writeAssetProbe() {
    try {
        const p = window.__preload;
        localStorage.setItem('asset_probe', JSON.stringify({
            base: window.__assetBase || '', total: p.total, done: p.done, errors: p.errors || 0,
            ua: navigator.userAgent.slice(0, 40), at: new Date().toISOString()
        }));
    } catch (e) { /* storage unavailable */ }
}
...
function dismissAppSplash() {
    ...
    writeAssetProbe();
    setTimeout(writeAssetProbe, 20000);   // again once the staggered preloads have all finished
...
function paintPoolGrid(host) {
    ...
    for (let i = 0; i < tileCount; i++) {
            ...
            return function () {
                ...
                function animate(now) {
                    ...
                    if (elapsed >= fadeInDuration) {
                        ...
                        finalImg.src = assetUrl('PoolImages/' + imgName);
                        finalImg.alt = '';
        ...
        imgSrc.decoding = 'async';
        imgSrc.src = assetUrl('PoolImages/' + imgName);
    }
}
...
function paintDigitalFireFighters(host) {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < perSet; c++) {
            ...
            const img = document.createElement('img');
            img.decoding = 'async';
            img.src = assetUrl('DigitalFireFighterImages/' + bgPool[(imgIdx++) % bgPool.length]);
            img.alt = '';
    ...
    for (let i = 0; i < orbitImages.length; i++) {
        const img = document.createElement('img');
        img.decoding = 'async';
        img.src = assetUrl('DigitalFireFighterImages/' + orbitImages[i]);
        img.className = 'orbit';
        img.alt = '';
...
function paintHodlWavesTiles(host) {
    ...
    for (let k = 0; k < 12; k++) {
        ...
        const img = document.createElement('img');
        img.decoding = 'async';
        img.src = assetUrl('WavesImages/' + shuffled[k]);
        img.alt = '';
...
function preloadSkinImagesToRAM(cacheKey, basePath, images) {
    ...
    (function next() {
        for (let k = 0; k < 6 && i < images.length; k++, i++) {
            const im = new Image();
            // Fetch only: this warms WebKit's memory cache (the ref keeps the entry live) so page
            // visits skip the .NET file read. Decoding is left to paint time (decoding='async')
            // instead of forcing ~750 WebP decodes during the first seconds after launch.
            im.onload = splashTrackOne;
            im.onerror = splashTrackError;
            im.src = assetUrl(basePath + images[i]);
            cache.push(im);             // keep a ref so the cache entry stays live
        }
    ...
    for (let c = 0; c < cols; c++) {
        ...
        for (let r = 0; r < 2; r++) {
            for (let i = 0; i < colImages.length; i++) {
                const img = document.createElement('img');
                img.decoding = 'async';
                img.src = assetUrl('NetworkImages/' + colImages[i]);
                img.alt = '';
                col.appendChild(img);
            }
        }
...
function paintCascadeGrid(host) {
    ...
    for (let i = 0; i < 64; i++) {
        ...
        const img = document.createElement('img');
        img.decoding = 'async';
        img.src = assetUrl('BlazecoinBird/' + selected[i]);
        img.alt = '';
        cell.appendChild(img);
        host.appendChild(cell);
    }
...
function paint413Tiles(host) {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            const img = document.createElement('img');
            img.decoding = 'async';
            img.src = assetUrl('Images/413/413-Vert.png');
            img.alt = '';
            row.appendChild(img);
        }
        host.appendChild(row);
...
function paintShieldTiles(host) {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            ...
            const img = document.createElement('img');
            img.decoding = 'async';
            img.src = assetUrl('shield-tile.png');
            img.alt = '';
...
function paintCoinGrid(host) {
    ...
    const img = new Image();
    img.decoding = 'async';
    img.src = assetUrl('BlazecoinBird/ComfyUI_02889_.webp');
    img.onload = function () {
        host.querySelectorAll('.pixel-coin').forEach(c => drawMirroredCoin(c, img, SIZE));
    };
}

// Grid coins (Pixelated Coins skin): a deliberately chunky size×size nearest-neighbour
// sample of the 512px art, shown with image-rendering: pixelated.
...
// Header coins: same nearest-neighbour pixel-art sampling as the grid coins, but the backing
// store follows devicePixelRatio (capped at 2, like the other canvases), so a 48px coin is
// 96 pixel-art samples on a Retina Mac instead of 48 samples blown up 2x (which read as a
// blocky, low-resolution coin on both Macs). Andrew picked this over smooth downsampling in a
// side-by-side, 2026-09-24. Display size is set in CSS (image-rendering: pixelated).
function drawMirroredCoinHiDpi(canvas, img, cssSize) {
...
window.typeBlazecoinBanner = function () {
    ...
    if (!el) return;
    // Idempotent per element: a repeat call on an already-typed banner (re-render, second
    // OnAfterRender) must not restart the animation or bring the caret back.
    if (el.dataset.typed === '1') return;
    const wrap = el.closest('.banner-text-wrap');
    const fullText = el.dataset.fullText || el.textContent.trim();
    el.dataset.fullText = fullText;
    el.textContent = '';
    // The caret is hidden by default in CSS and shown only while the wrapper is .typing, so a
    // caret recreated by a later render comes back hidden (Intel Mac, 2026-09-24).
    ...
    window.__bannerTypeTimer = setInterval(() => {
        if (i >= fullText.length) {
            ...
            el.dataset.typed = '1';
            setTimeout(() => { if (wrap) wrap.classList.remove('typing'); }, 800);
            return;
        }
...
window.drawHeaderCoins = function () {
    ...
    img.onload = function () {
        document.querySelectorAll('.header-coin').forEach(c => {
            drawMirroredCoinHiDpi(c, img, SIZE);
            c.dataset.drawn = '1';
        });
    };
};
...
window.DashboardSkins = {
    ...
    // The Quantum Exposure page's background, as a dashboard skin (same painter).
    'quantum': {
        id: 'quantum',
        label: 'Quantum',
        init(host) {
            paintQuantumTiles(host);
        }
    },
    'plain': {
        id: 'plain',
...
function paintCoinPileDiagonal(host) {
    ...
    for (let r = 0; r < totalRows; r++) {
        for (let c = 0; c < totalCols; c++) {
            ...
            img.decoding = 'async';
            img.src = assetUrl('CoinPilesImages/' + base[(r % rowsView) * colsView + (c % colsView)]);
            img.alt = '';
...
function paintBackupDiagonal(host) {
    ...
    for (let r = 0; r < totalRows; r++) {
        for (let c = 0; c < totalCols; c++) {
            ...
            // diagonal scroll wraps with no visible seam.
            img.decoding = 'async';
            img.src = assetUrl('BackupImages/' + base[(r % rowsView) * colsView + (c % colsView)]);
            img.alt = '';
...
// The QUANTUM page's skin (#quantumTileBg): a solid black ground; a mosaic of 240px tiles with crimson grout, each
// cell one of the 30 rendered atoms in wwwroot/QuantumImages/atoms/ on a 6x6 repeating pattern (QX_TILE_ORDER), the
// sheet drifting bottom-left to top-right one tile every 12s and looping seamlessly every 6 tiles; a soft sheen sweeping the same way;
...
// The 30 rendered atoms (wwwroot/QuantumImages/atoms/atom-01..30.webp), warmed into RAM like the other image skins.
const QX_ATOM_IMAGES = Array.from({ length: 30 }, (_, i) => 'atom-' + (i < 9 ? '0' : '') + (i + 1) + '.webp');

// Paints the Quantum skin into any host. Used by the Quantum Exposure page (#quantumTileBg) and, as the
// "Quantum" Dashboard skin, by #dashboardBackground and the Preferences preview (#prefBackground).
function paintQuantumTiles(host) {
    if (!host) return;
    if (host.dataset.populated === '1') return;
    host.dataset.populated = '1';
    host.className = 'skin-quantum-tiles';
    preloadSkinImagesToRAM('quantum', 'QuantumImages/atoms/', QX_ATOM_IMAGES);   // idempotent
    ...
    const field = document.createElement('div'); field.className = 'qx-atom-canvas qx-atom-field';
    host.appendChild(icons); host.appendChild(sheen); host.appendChild(field);

    // 6x6 pattern period over atom-01..30 (six repeats placed so no image touches itself, wrap included).
    // The CSS drifts the sheet exactly one period (6 x 240px) per loop, so the loop has no visible jump.
    const QX_TILE = 240, QX_PERIOD = 6;
    const QX_TILE_ORDER = [1, 14, 27, 10, 23, 6, 19, 2, 15, 28, 11, 24, 7, 20, 3, 16, 29, 12,
                           25, 8, 21, 4, 17, 30, 13, 26, 9, 22, 5, 18, 3, 16, 29, 12, 25, 8];
    function buildTiles(w, h) {
        const span = QX_TILE * QX_PERIOD;                 // one drift period
        const left = -(span + QX_TILE), top = -QX_TILE;   // start left of and above the viewport by whole tiles
        const cols = Math.ceil((w + span + 2 * QX_TILE) / QX_TILE);
        const rows = Math.ceil((h + span + 2 * QX_TILE) / QX_TILE);
        icons.style.left = left + 'px'; icons.style.top = top + 'px';
        icons.style.width = (cols * QX_TILE) + 'px'; icons.style.height = (rows * QX_TILE) + 'px';
        const frag = document.createDocumentFragment();
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const n = QX_TILE_ORDER[(r % QX_PERIOD) * QX_PERIOD + (c % QX_PERIOD)];
                const tile = document.createElement('div'); tile.className = 'qx-tile';
                tile.style.backgroundImage = "url('QuantumImages/atoms/atom-" + (n < 10 ? '0' + n : n) + ".webp')";
                frag.appendChild(tile);
            }
        }
        icons.replaceChildren(frag);
    }

    // Atoms (2026-09-24, Intel Mac "jarring"): originally one full-screen canvas at the device pixel
    // ratio, cleared and redrawn every frame. On the Intel Mac's WebKit that canvas is rasterised in
    // software and its ~27 MB of pixels re-uploaded per frame, so the page ran at ~25 fps however
    // little was drawn (measured: pausing the canvas alone took it to 54 fps). Now each atom is a
    // small canvas rendered ONCE (three orbit ellipses + glowing nucleus) plus three electron sprites
    // rendered once, and the frame loop only writes CSS transforms, which the compositor animates.
    // Shapes, radii, colours, glow blurs and motion are the same as the canvas version.
    const reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    let atoms = [], W = 0, H = 0, paused = false;
    const seed = i => { const x = Math.sin(i * 9301 + 49297) * 233280; return x - Math.floor(x); };
    function sprite(half, dpr, draw) {
        const c = document.createElement('canvas');
        const size = Math.ceil(half * 2 * dpr);
        c.width = size; c.height = size;
        c.style.width = (size / dpr) + 'px'; c.style.height = (size / dpr) + 'px';
        c.style.left = (-size / dpr / 2) + 'px'; c.style.top = (-size / dpr / 2) + 'px';
        const x = c.getContext('2d');
        x.setTransform(dpr, 0, 0, dpr, size / 2, size / 2);
        draw(x);
        return c;
    }
    function buildAtom(a, dpr) {
        const el = document.createElement('div'); el.className = 'qx-atom';
        const er = Math.max(2.2, a.r * 0.045), ns = Math.max(1, a.r / 48);
        // body: the three tilted orbit ellipses and the nucleus, exactly as drawAtom painted them
        el.appendChild(sprite(a.r + 20, dpr, function (x) {
            for (let k = 0; k < 3; k++) {
                x.save(); x.rotate(k * Math.PI / 3);
                x.beginPath(); x.ellipse(0, 0, a.r, a.r * 0.38, 0, 0, Math.PI * 2);
                x.strokeStyle = 'rgba(255, 90, 90, 0.22)'; x.lineWidth = Math.max(1, a.r / 48); x.stroke();
                x.restore();
            }
            x.shadowColor = 'rgba(207, 32, 19, 0.9)'; x.shadowBlur = 14;
            const n = [[0, 0], [3.2 * ns, -1.8 * ns], [-2.6 * ns, 2.4 * ns], [1.4 * ns, 3.4 * ns]];
            for (let j = 0; j < n.length; j++) {
                x.beginPath(); x.arc(n[j][0], n[j][1], 3 * ns, 0, Math.PI * 2);
                x.fillStyle = j % 2 ? '#ffd9d4' : '#cf2013'; x.fill();
            }
        }));
        a.electrons = [];
        for (let k = 0; k < 3; k++) {
            const e = document.createElement('div'); e.className = 'qx-electron';
            e.appendChild(sprite(er + 10, dpr, function (x) {
                x.beginPath(); x.arc(0, 0, er, 0, Math.PI * 2);
                x.fillStyle = '#ff6a5a'; x.shadowColor = 'rgba(255, 90, 90, 0.9)'; x.shadowBlur = 8; x.fill();
            }));
            el.appendChild(e); a.electrons.push(e);
        }
        a.el = el;
    ...
    function layout() {
        ...
        const count = Math.max(8, Math.round((W * H) / 110000));   // fewer, since each atom is ~1.6x larger
        atoms = [];
        for (let i = 0; i < count; i++) {
            atoms.push({
                x: seed(i * 3 + 1) * W, y: seed(i * 3 + 2) * H,
                r: 40 + seed(i * 3 + 3) * 72,                 // foreground atom radius 40-112px
            ...
            buildAtom(atoms[i], dpr);
        }
    }
    function placeAtom(a, t) {
        const s = t * a.v, m = a.r + 12;
        const x = ((a.x + s + m) % (W + 2 * m)) - m;
        const y = H - (((H - a.y) + s + m) % (H + 2 * m)) + m;
        const wob = Math.sin(t * 0.7 + a.wob) * 6;
        a.el.style.transform = 'translate3d(' + (x + wob) + 'px,' + (y + wob) + 'px,0) rotate(' + (a.tilt + t * 0.03 * a.speed) + 'rad)';
        for (let k = 0; k < 3; k++) {
            const ang = a.phase + t * a.speed * 1.6 + k * 2.1;
            a.electrons[k].style.transform = 'rotate(' + (k * 60) + 'deg) translate3d(' + (Math.cos(ang) * a.r) + 'px,' + (Math.sin(ang) * a.r * 0.38) + 'px,0)';
        }
    }
    function frame(ts) {
        if (!field.isConnected) { window.removeEventListener('resize', layout); return; }
        if (!paused) { const t = ts / 1000; for (let i = 0; i < atoms.length; i++) placeAtom(atoms[i], t); }
        if (!reduce) requestAnimationFrame(frame);
    }
    layout(); window.addEventListener('resize', layout);
    requestAnimationFrame(frame);
    // Diagnostic (2026-09-24): 15 s of frame-gap stats after mount, bucketed by whether the banner's
    // decrypt effect is mid-scramble (window.__qxBannerRaf set) or holding -> localStorage 'qx_probe',
    // readable from outside the app like nav_probe. Passive: it never pauses anything.
    (function () {
        const b = { scramble: { sum: 0, n: 0, worst: 0, over33: 0 }, hold: { sum: 0, n: 0, worst: 0, over33: 0 } };
        let last = null; const t0 = performance.now();
        function tick(now) {
            if (!field.isConnected) return;
            if (last !== null) {
                const g = now - last, k = window.__qxBannerRaf ? b.scramble : b.hold;
                k.sum += g; k.n++; if (g > k.worst) k.worst = g; if (g > 33) k.over33++;
            }
            last = now;
            if (now - t0 < 15000) { requestAnimationFrame(tick); return; }
            const fmt = k => ({ avgMs: k.n ? Math.round(k.sum / k.n * 10) / 10 : null, worstMs: Math.round(k.worst), over33: k.over33, frames: k.n });
            try { localStorage.setItem('qx_probe', JSON.stringify({ scramble: fmt(b.scramble), hold: fmt(b.hold), atoms: atoms.length, w: W, h: H, tiles: icons.childElementCount, mode: 'dom', at: new Date().toISOString() })); } catch (e) { }
        }
        requestAnimationFrame(tick);
    })();
}

// The Quantum Exposure page's own background host.
window.initQuantumBackground = function () {
    paintQuantumTiles(document.getElementById('quantumTileBg'));
};
...
window.decryptQuantumBanner = function () {
    ...
    const run = () => {
        ...
        let start = null, lastScramble = -1;
        const tick = ts => {
            if (!document.body.contains(el)) return;
            if (start === null) start = ts;
            const resolved = Math.floor(((ts - start) / 45) / perChar);
            // Re-randomise the cipher glyphs every 50 ms rather than on every frame: changing every
            // glyph 60 times a second read as flicker/jitter on an Intel Mac (2026-09-24). Letters
            // still resolve left to right on the same 45 ms x perChar cadence.
            const scramble = ts - lastScramble >= 50;
            if (scramble) lastScramble = ts;
            for (let i = 0; i < n; i++) {
                ...
                else if (scramble || !cell.classList.contains('cipher')) { cell.classList.add('cipher'); cell.textContent = GLYPHS[Math.floor(Math.random() * GLYPHS.length)]; }
            }
