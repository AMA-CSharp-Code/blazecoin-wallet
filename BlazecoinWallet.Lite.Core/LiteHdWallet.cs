...
public sealed class LiteHdWallet
{
    ...
    private readonly byte[] _pqMasterSeed;
    ...
    private LiteHdWallet(Mnemonic mnemonic, string passphrase, Network? network)
    {
        ...
        // The post-quantum chain descends from the SAME BIP39 seed (PQ_SIGNATURES.md §6):
        // master_seed = HMAC-SHA512(key "Blazecoin PQ", data = 64-byte seed)[0:32]. The
        // mnemonic (+ passphrase) therefore already backs up every future BQ address.
        _pqMasterSeed = Pq.PqKeyDerivation.MasterSeedFromBip39Seed(mnemonic.DeriveSeed(passphrase));
    ...
    // ── Post-quantum chain (P2PQH, PQ_SIGNATURES.md §6) ─────────────────────────────────

    /// <summary>The 32-byte PQ master seed (secret) — the root of every ML-DSA-44 key.</summary>
    internal ReadOnlySpan<byte> PqMasterSeed => _pqMasterSeed;

    /// <summary>The ML-DSA-44 key at PQ slot <paramref name="index"/>: ξ_i =
    /// TaggedHash("Blazecoin/MLDSA44/seed", master_seed ‖ uint32_le(i)), then FIPS 204
    /// KeyGen_internal. Deterministic, so a restore recovers every slot. Each call derives
    /// afresh (~1 ms); callers resolving many keys should keep the result.</summary>
    public Pq.PqKey GetPqKey(int index) => Pq.PqKeyDerivation.Derive(_pqMasterSeed, index);

    /// <summary>The BQ (mainnet) / TQ (regtest) address at PQ slot <paramref name="index"/>.</summary>
    public string GetPqAddress(int index) => GetPqKey(index).Address(_network);
}
