...
public sealed class LiteTxVerifier
{
        ...
        string? expectedAddress = null, CancellationToken ct = default)
    {
        ...
        {
            string? owner = null;
            try { owner = fundingTx.Outputs[vout].ScriptPubKey.GetDestinationAddress(_network)?.ToString(); }
            catch { /* non-standard script → owner stays null → mismatch below */ }
            if (!string.Equals(owner, expectedAddress, StringComparison.Ordinal))
