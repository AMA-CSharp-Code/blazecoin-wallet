...
/// <param name="ScriptPubKey">The output's scriptPubKey hex when the daemon served it (rows
/// indexed before this column existed carry null; a wallet derives the script from the
/// address in that case).</param>
public sealed record ConfirmedUtxo(string TxId, int Vout, long Amount, long BlockHeight, bool IsCoinbase, string? ScriptPubKey = null);
...
public sealed class UtxoIndexDb
{
    ...
    /// <param name="mainnet">Which prefix a gateway-derived P2PQH address gets (BQ mainnet /
    /// TQ regtest) — see <see cref="OutputAddressResolver"/>. Legacy addresses come from the
    /// daemon already encoded, so this touches nothing else.</param>
    public UtxoIndexDb(string dbPath, bool mainnet = true)
    {
        _mainnet = mainnet;
        ...
        Exec(db, """
            ...
            """);
        // Schema migration for an index built before the scriptPubKey column existed: add it
        // (nullable, no rewrite). Older rows stay null; the walker fills new rows from here on.
        if (!HasColumn(db, "outputs", "script_pubkey"))
            Exec(db, "ALTER TABLE outputs ADD COLUMN script_pubkey TEXT;");
    }
    ...
    public void ApplyBlock(RpcBlock block)
    {
        ...
        for (var i = 0; i < block.Tx.Count; i++)
        {
            ...
            {
                // The daemon's address, or — for a P2PQH template the daemon didn't decode —
                // the BQ/TQ address derived here from the script hex.
                var address = OutputAddressResolver.Resolve(vout.ScriptPubKey, _mainnet);
                if (address == null) continue; // non-standard / OP_RETURN — not spendable-by-address
                using var ins = db.CreateCommand();
                ins.Transaction = tx;
                ins.CommandText = """
                    INSERT OR REPLACE INTO outputs(txid, vout, address, amount, block_height, is_coinbase, spent_txid, spent_height, script_pubkey)
                    VALUES($txid, $vout, $addr, $amount, $height, $cb, NULL, NULL, $spk)
                    """;
                ...
                ins.Parameters.AddWithValue("$spk", (object?)OutputAddressResolver.NormalizeHex(vout.ScriptPubKey) ?? DBNull.Value);
                ins.ExecuteNonQuery();
            }
    ...
    public List<ConfirmedUtxo> GetUtxos(string address)
    {
        ...
        using var cmd = db.CreateCommand();
        cmd.CommandText = """
            SELECT txid, vout, amount, block_height, is_coinbase, script_pubkey FROM outputs
