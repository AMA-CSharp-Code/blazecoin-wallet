...
public sealed class ChainWalkerService : BackgroundService
{
    ...
    /// <summary>Configuration key selecting the chain the daemon runs ("mainnet" default,
    /// "regtest") — it only decides the BQ/TQ prefix of gateway-derived P2PQH addresses.</summary>
    public const string NetworkConfigKey = "Index:Network";

    public static bool IsMainnet(IConfiguration config) =>
        !string.Equals(config[NetworkConfigKey], "regtest", StringComparison.OrdinalIgnoreCase);
    ...
    private async Task RefreshMempoolAsync(CancellationToken ct)
    {
        ...
        foreach (var t in _mempoolTxCache.Values)
        {
            ...
            {
                // Template-aware (a P2PQH output the daemon didn't decode still lands under its BQ address).
                var address = OutputAddressResolver.Resolve(vout.ScriptPubKey, _mainnet);
                if (address != null)
                    outputs.Add(new MempoolUtxo(t.TxId, vout.N, address, vout.Satoshis, OutputAddressResolver.NormalizeHex(vout.ScriptPubKey)));
            }
