...
/// costs nothing and regtest fixtures sometimes carry old shapes. <paramref name="Hex"/> is
/// the raw scriptPubKey: the index needs it to recognise the post-quantum P2PQH template
/// (which a pre-fork daemon serves WITHOUT an address) and to hand wallets the exact script
/// their signatures must commit to.</summary>
...
{
    /// <summary>The address the DAEMON decoded (legacy types only) — see
    /// <c>Index.OutputAddressResolver</c> for the template-aware resolution the index uses.</summary>
