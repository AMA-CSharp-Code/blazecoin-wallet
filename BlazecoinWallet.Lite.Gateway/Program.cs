...
builder.Services.AddSingleton(sp => new UtxoIndexDb(
    ...
    mainnet: ChainWalkerService.IsMainnet(builder.Configuration)));
...
// PQ_SIGNATURES §7.1: the same response carries the fork announcement — forkName /
// forkHeight / minClientVersion from Fork:* configuration, all null until the operator sets
// them the day H_Q is chosen (gw and gw2 identically). Read per request, so a config reload
// (appsettings.json is reloadOnChange) reaches clients without a restart. The lite heads poll
// this every 30 s under the "indexer/api" spelling they use for everything else, so it is
// mapped under both prefixes like the wallet surface.
static object StatusPayload(WalkerStatus status, MempoolOverlay mempool, IConfiguration config)
{
    var forkName = config.GetValue<string?>("Fork:Name", null);
    var minClient = config.GetValue<string?>("Fork:MinClientVersion", null);
    return new
    {
        ...
        forkName = string.IsNullOrWhiteSpace(forkName) ? null : forkName.Trim(),
        forkHeight = config.GetValue<long?>("Fork:Height", null),
        minClientVersion = string.IsNullOrWhiteSpace(minClient) ? null : minClient.Trim(),
    };
}
app.MapGet("/api/status", (WalkerStatus status, MempoolOverlay mempool, IConfiguration config)
    => Results.Ok(StatusPayload(status, mempool, config)));
app.MapGet("/indexer/api/status", (WalkerStatus status, MempoolOverlay mempool, IConfiguration config)
    => Results.Ok(StatusPayload(status, mempool, config)));
    ...
    api.MapGet("/address/{address}/utxos", (string address, UtxoIndexDb db, MempoolOverlay mempool, WalkerStatus status) =>
    {
        ...
        var confirmed = db.GetUtxos(address)
            ...
            .Select(u => new UtxoResponse(u.TxId, u.Vout, u.Amount,
                (int)Math.Max(1, tip - u.BlockHeight + 1), u.BlockHeight, u.IsCoinbase, u.ScriptPubKey));
        var unconfirmed = mempool.GetForAddress(address)
            .Select(u => new UtxoResponse(u.TxId, u.Vout, u.Amount, 0, 0, false, u.ScriptPubKey));
...
/// <summary>Field-for-field the full Indexer's UtxoDto, plus the output's scriptPubKey hex
/// (null when the daemon didn't serve it) so a wallet can see the exact template a coin sits
/// on — a P2PQH coin needs an ML-DSA signature, not an ECDSA one.</summary>
