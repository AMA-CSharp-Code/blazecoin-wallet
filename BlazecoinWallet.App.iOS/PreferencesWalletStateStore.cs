...
public sealed class PreferencesWalletStateStore : IWalletStateStore
{
    ...
    private const string PqKey = "pq_address_count";
    ...
    public Task<int> GetPqAddressCountAsync()
        => Task.FromResult(Preferences.Default.Get(PqKey, 0));

    public Task SetPqAddressCountAsync(int count)
    {
        Preferences.Default.Set(PqKey, count);
        return Task.CompletedTask;
    }
    ...
    public Task ClearAsync()
    {
        ...
        Preferences.Default.Remove(PqKey);
        return Task.CompletedTask;
    }
}
