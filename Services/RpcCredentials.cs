...
public sealed class RpcCredentials
{
    ...
    /// <summary>Mirror of blazecoind's <c>GetDefaultDataDir()</c> (args.cpp): the legacy
    /// Roaming datadir wins if it exists; fresh installs live under Local. The wallet
    /// MUST resolve identically to the daemon or a zero-config install reads its
    /// <c>.cookie</c> from a directory the daemon never writes to.</summary>
    internal static string DefaultDataDir()
    {
        var roaming = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "BlazecoinV2.0");
        if (Directory.Exists(roaming)) return roaming;
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "BlazecoinV2.0");
    }
}
