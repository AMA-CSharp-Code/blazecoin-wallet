...
public sealed class DaemonProcessManager : IDaemonProcessManager
{
    ...
    public async Task EnsureRunningAsync()
    {
        ...
        if (exe is null)
        {
            ...
            _log?.LogInformation("No blazecoind.exe configured or found next to the app — not auto-launching.");
            return;
        }
    ...
    public async Task StopIfOwnedAsync()
    {
        ...
        //    A console CTRL_BREAK triggers a GRACEFUL shutdown even during the load (the
        //    load loop checks ShutdownRequested()), so the node still flushes cleanly.
        if (TrySendCtrlBreak() && await WaitForExitAsync(TimeSpan.FromSeconds(15))) return;
        ...
        try
        {
            _log?.LogInformation("Graceful stop + CTRL_BREAK didn't complete — terminating the daemon we started.");
    ...
    // blazecoind.exe path: explicit config wins; otherwise look next to the app (where
    // a bundled install ships it). Null → nothing to launch.
    private string? ResolveExePath()
    {
        ...
        var beside = Path.Combine(AppContext.BaseDirectory, "blazecoind.exe");
